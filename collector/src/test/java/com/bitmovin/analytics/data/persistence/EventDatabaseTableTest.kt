...
class EventDatabaseTableTest {
    ...
    @Test
    fun `creating the table it is empty`() =
        databaseTest {
            assertThat(pop()).isNull()
        }

    @Test
    fun `popping an element returns the pushed element`() =
        databaseTest {
    ...
    @Test
    fun `popping multiple elements returns the pushed element in the same order`() =
        databaseTest {
            val pushedEvents =
                listOf(
    ...
    @Test
    fun `purging deletes all elements`() =
        databaseTest {
            val pushedEvents =
                listOf(
                    createRandomEventDatabaseEntry(),
    ...
    @Test
    fun `finding purgeable sessions lists sessions that started before the age limit`() =
        databaseTest {
            ...
            val purgeableSession =
                findPurgeableSessions(
                    RetentionConfig(
                        ageLimit = 1.seconds,
    ...
    @Test
    fun `finding purgeable sessions lists an old session when the overall count exceeds the count limit`() =
        databaseTest {
            every { Util.timestamp } returns 4000
            listOf(
                createRandomEventDatabaseEntry(sessionId = "session1", eventTimestamp = 1000),
    ...
    @Test
    fun `deleting sessions removes all entries with the according session ids from the database`() =
        databaseTest {
...
private fun DatabaseTableTest.push(event: EventDatabaseEntry) =
...
private fun DatabaseTableTest.findPurgeableSessions(retentionConfig: RetentionConfig) =
    database.transaction {
        table.findPurgeableSessions(this, retentionConfig)
    }

private fun DatabaseTableTest.deleteSessions(sessionIds: List<String>) =
    database.transaction {
        table.deleteSessions(this, sessionIds)
    }

private fun databaseTest(block: DatabaseTableTest.() -> Unit) {
