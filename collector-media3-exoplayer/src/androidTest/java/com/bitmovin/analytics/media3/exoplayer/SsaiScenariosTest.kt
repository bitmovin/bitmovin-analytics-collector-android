...
class SsaiScenariosTest {
    ...
    fun test_adEngagementMetrics_track_adbreakabandonment_metadata_when_adbreak_completes() =
        runBlockingTest {
            ...
            Media3PlayerPlaybackUtils.waitUntilPlayerHasPlayedToMs(player, 200)

            withContext(mainScope.coroutineContext) {
                collector.ssai.adQuartileFinished(SsaiAdQuartile.FIRST)
            }

            Media3PlayerPlaybackUtils.waitUntilPlayerHasPlayedToMs(player, 400)

            withContext(mainScope.coroutineContext) {
                collector.ssai.adQuartileFinished(SsaiAdQuartile.MIDPOINT)
            }

            Media3PlayerPlaybackUtils.waitUntilPlayerHasPlayedToMs(player, 600)

            withContext(mainScope.coroutineContext) {
                collector.ssai.adQuartileFinished(SsaiAdQuartile.THIRD)
            }

            Media3PlayerPlaybackUtils.waitUntilPlayerHasPlayedToMs(player, 800)
            ...
            Media3PlayerPlaybackUtils.waitUntilPlayerHasPlayedToMs(player, 1000)

            withContext(mainScope.coroutineContext) {
                collector.ssai.adQuartileFinished(SsaiAdQuartile.FIRST)
            }

            Media3PlayerPlaybackUtils.waitUntilPlayerHasPlayedToMs(player, 1200)

            withContext(mainScope.coroutineContext) {
                collector.ssai.adQuartileFinished(SsaiAdQuartile.MIDPOINT)
            }

            Media3PlayerPlaybackUtils.waitUntilPlayerHasPlayedToMs(player, 1400)

            withContext(mainScope.coroutineContext) {
                collector.ssai.adQuartileFinished(SsaiAdQuartile.THIRD)
            }

            Media3PlayerPlaybackUtils.waitUntilPlayerHasPlayedToMs(player, 1600)
            ...
            Media3PlayerPlaybackUtils.waitUntilPlayerHasPlayedToMs(player, 1800)
            ...
            Media3PlayerPlaybackUtils.waitUntilPlayerHasPlayedToMs(player, 2200)

            withContext(mainScope.coroutineContext) {
                collector.ssai.adQuartileFinished(SsaiAdQuartile.THIRD)
            }

            Media3PlayerPlaybackUtils.waitUntilPlayerHasPlayedToMs(player, 2400)
    ...
    fun test_adEngagementMetrics_sends_adabandonment_metadata_on_player_destroy() =
        runBlockingTest {
            ...
            withContext(mainScope.coroutineContext) {
                // Media3's Player has no release listener the collector can hook into,
                // so the user contract is to detach the collector before releasing the
                // player. detachPlayer flushes the in-flight SSAI ad sample with
                // exitedAdBreak=true.
                collector.detachPlayer()
                player.release()
            }
