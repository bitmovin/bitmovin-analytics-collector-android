...
// System test for programChange scenarios using the bundled bitmovin player
// (player created with an AnalyticsConfig instead of a standalone collector).
@RunWith(AndroidJUnit4::class)
class BundledAnalyticsProgramChangeTest {
    ...
    private fun buildSource(sourceMetadata: SourceMetadata) =
        SourceBuilder(SourceConfig.fromUrl(hlsSample.m3u8Url!!)).configureAnalytics(sourceMetadata).build()
    ...
    fun test_programChange_duringStartup_ShouldOnlySetMetadataAndNotSendAdditionalSamples() =
        runBlockingTest {
            withContext(mainScope.coroutineContext) {
                player.load(buildSource(sourceMetadataProgram1))
            }
            ...
            withContext(mainScope.coroutineContext) {
                player.analytics?.programChange(sourceMetadataProgram2)
            }
    ...
    fun test_programChange_multipleTimes_createsMultipleSessions() =
        runBlockingTest {
            ...
            withContext(mainScope.coroutineContext) {
                player.analytics?.programChange(sourceMetadataProgram3)
            }
    ...
    fun test_programChange_withPlaylist_onlyUpdatesActiveSourceMetadata() =
        runBlockingTest {
            ...
            // Call programChange - this should only update the active (HLS) source metadata
            withContext(mainScope.coroutineContext) {
                player.analytics?.programChange(programChangeMetadata)
            }
