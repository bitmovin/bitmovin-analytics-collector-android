...
class PhoneBasicScenariosTest {
    ...
    fun test_live_playWithAutoplay() =
        runBlockingTest {
            ...
            DataVerifier.verifyStartupSample(eventDataList[0])
            DataVerifier.verifyVideoStartEndTimesOnContinuousPlayback(eventDataList)
            DataVerifier.verifyPlayerSetting(eventDataList, PlayerSettings(false, true))
            DataVerifier.verifyInvariants(eventDataList)
    ...
    fun test_vodHls_seekForwardsAndBackwardsWhilePlaying() =
        runBlockingTest {
            ...
            ExoPlayerPlaybackUtils.waitUntilPlayerHasPlayedToMs(player, 11000)
    ...
    @Test
    fun test_vod_enableGermanSwitchEnglishAndDisableSubtitles() =
        runBlockingTest {
            // arrange
            val collector = IExoPlayerCollector.create(appContext, defaultAnalyticsConfig)
    ...
    fun test_vodDash_seekWhilePaused() =
        runBlockingTest {
            ...
            ExoPlayerPlaybackUtils.waitUntilPlayerHasPlayedToMs(player, 63000)
